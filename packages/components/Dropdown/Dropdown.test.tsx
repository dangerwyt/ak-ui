...
import { AkaDropdown, AkaDropdownItem } from '.';
...
describe('Dropdown/index', () => {
  ...
  it('should be exported with withInstall', () => {
    expect(AkaDropdown.install).toBeDefined();
    expect(AkaDropdownItem.install).toBeDefined();
  });
  // 测试 Dropdown 组件是否被正确导出
  it('should be exported Dropdown component', () => {
    expect(AkaDropdown).toBe(Dropdown);
    expect(AkaDropdownItem).toBe(DropdownItem);
  });

  // 可选：测试 withInstall 是否增强了 Dropdown 组件的功能
  test('should enhance Dropdown component', () => {
    const enhancedDropdown = withInstall(Dropdown);
    expect(enhancedDropdown).toBe(AkaDropdown);
...
describe('Dropdown.vue', () => {
  ...
  it('it should render slots correctly', () => {
    const itmes: DropdownItemProps[] = [
    ...
    const wrapper = mount(Dropdown, {
      ...
      slots: {
        ...
        dropdown: () => itmes.map((item) => <DropdownItem {...item} />),
      },
    });
    expect(wrapper.text()).toContain('Default slot content');
    expect(wrapper.find('.aka-dropdown').exists()).toBeTruthy();
  });
  ...
  it('should emit command event when item is clicked', async () => {
    ...
    expect(wrapper.find('.aka-dropdown__menu').exists()).toBeTruthy();
  ...
  it('should toggle visibility when split btn is clicked', async () => {
    ...
    expect(wrapper.find('.aka-dropdown__menu').exists()).toBeFalsy();
